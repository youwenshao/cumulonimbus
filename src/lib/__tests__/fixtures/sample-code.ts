...
/**
 * Code with CommonJS require() calls (generated by some LLMs)
 */
export const codeWithCommonJS = `const React = require('react');
...
function App() {
  ...
  return React.createElement('div', { className: 'p-4' }, [
    React.createElement('h1', { key: 'title' }, 'Counter App'),
    React.createElement('p', { key: 'count' }, \`Count: \${count}\`),
    React.createElement('p', { key: 'date' }, \`Date: \${format(date, 'yyyy-MM-dd')}\`),
    React.createElement('button', {
      key: 'increment',
      onClick: () => setCount(count + 1)
    }, [React.createElement(Plus, { key: 'icon', size: 16 }), ' Increment'])
  ]);
}
...
/**
 * Code with destructured CommonJS requires
 */
export const codeWithDestructuredCommonJS = `const { useState, useEffect, useCallback } = require('react');
